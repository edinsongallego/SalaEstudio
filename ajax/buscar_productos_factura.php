...
$sql="SELECT * FROM (SELECT *, if(ISNULL(CI),0,CI) CANTIDAD_INVENTARIO FROM (SELECT P.*, TU.DS_NOMBRE_UNIDAD, TU.DS_DESCRIPCION_UNIDAD, (SELECT SUM(NM_CANTIDAD_INVENTARIO) FROM tp_inventario_producto WHERE CS_PRODUCTO_ID = P.CS_PRODUCTO_ID GROUP BY CS_PRODUCTO_ID ) CI FROM  tp_producto P INNER JOIN tp_unidad_medida_producto TU ON P.FK_UNIDAD = TU.CS_UNIDAD_ID WHERE P.CS_PRODUCTO_ID NOT IN (4,5) AND NM_ELIMINADO = 0 AND P.CS_PRODUCTO_ID NOT IN(".$id_productos.") AND (P.DS_CODIGO_PRODUCTO LIKE '%".$term."%' OR P.DS_NOMBRE_PRODUCTO LIKE '%".$term."%' OR P.DS_DESCRIPCION_PRODUCTO LIKE '%".$term."%')) tmp ) tmp1 ORDER BY CANTIDAD_INVENTARIO DESC LIMIT 10";
...
while ($row=mysqli_fetch_array($query)){ ?>
	<tr>
		...
		<td><?php echo $row["CANTIDAD_INVENTARIO"]; ?></td>
                <td><input type="number" id_prodcuto="<?php echo $row['CS_PRODUCTO_ID']; ?>" class="form-control ignoreClass" min="1" required data-rule-cantidadExistenteProductos="true" id="cantidad_<?php echo $row['CS_PRODUCTO_ID']; ?>" name="cantidad_<?php echo $row['CS_PRODUCTO_ID']; ?>"/></td>
		<td><input type="number" id_prodcuto="<?php echo $row['CS_PRODUCTO_ID']; ?>" class="form-control ignoreClass" required min="1" id="precio_<?php echo $row['CS_PRODUCTO_ID']; ?>" value="<?php echo $row['DB_PRECIO_VENTA_UND']; ?>"/></td>
